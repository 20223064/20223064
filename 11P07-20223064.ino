...
void loop() {
  ...
  if (dist_raw < _DIST_MIN) {
    dist_raw = dist_prev-10.0;           // cut lower than minimum
    digitalWrite(PIN_LED, 1);       // LED OFF
  } else if (dist_raw >= _DIST_MAX) {
    dist_raw = dist_prev+10.0;           // Cut higher than maximum
    digitalWrite(PIN_LED, 1);       // LED OFF
  ...
  if (dist_ema <=  _TARGET_LOW) {
    myservo.writeMicroseconds( _DUTY_MIN);      // LED OFF
  } else if (dist_ema < _TARGET_HIGH and dist_ema>_TARGET_LOW ) {
    myservo.writeMicroseconds(((dist_ema-180.0)*(_DUTY_MAX-_DUTY_MIN)/180.0)+550);       // LED OFF
  } else if (dist_ema >= _TARGET_HIGH){  // In desired Range
